      ...
      <div className='container max-w-3xl py-6'>
        <div className='flex items-center justify-between mb-6'>
          ...
          <div className='flex gap-3'>
            <Badge
              variant={
                deed?.data?.metadata?.status?.toLowerCase() === 'paid' ? 'default' : 'destructive'
              }>
              {deed?.data?.metadata?.status || 'unpaid'}
            </Badge>
        ...
        {deed?.data?.metadata?.status !== 'paid' && (
          <PaymentButton agreementId={deedId} agreementType='deed of assignment' />
        )}
