      ...
      <div className='container max-w-3xl py-6'>
        <div className='flex items-center justify-between mb-6'>
          ...
          <div className='flex gap-3'>
            <Badge
              variant={
                lease?.data?.metadata?.status?.toLowerCase() === 'paid' ? 'default' : 'destructive'
              }>
              {lease?.data?.metadata?.status || 'unpaid'}
            </Badge>
        ...
        {lease?.data?.metadata?.status !== 'paid' && (
          <PaymentButton agreementId={leaseId} agreementType='lease' />
        )}
