      ...
      <div className='container max-w-3xl py-6'>
        <div className='flex flex-col md:flex-row  items-start md:items-center justify-between mb-6'>
          ...
          <div className='flex gap-3'>
            <Badge
              variant={
                tenancy?.data?.metadata?.status?.toLowerCase() === 'paid'
                  ? 'default'
                  : 'destructive'
              }>
              {tenancy?.data?.metadata?.status || 'unpaid'}
            </Badge>
        ...
        {tenancy?.data?.metadata?.status !== 'paid' && (
          <PaymentButton agreementId={tenancyId} agreementType='tenancy' />
        )}
